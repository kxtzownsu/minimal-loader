...
__again:
  ...
  uint8_t pubkey[256];
  if (!rsa || EVP_PKEY_base_id(key) != EVP_PKEY_RSA || EVP_PKEY_size(key) != 256) {
    goto __fail;
  }

  RSA_get0_key(rsa, &n, nullptr, nullptr);
  if (!n || BN_bn2bin(n, pubkey) != 256) {
    goto __fail;
  }
...
int Gnubby::write(RSA* rsa) {
  ...
  BIGNUM *p_dup = nullptr, *q_dup = nullptr, *dmp1_dup = nullptr;
  BIGNUM *dmq1_dup = nullptr, *iqmp_dup = nullptr, *n_dup = nullptr, *e_dup = nullptr;

  const BIGNUM *p, *q, *dmp1, *dmq1, *iqmp, *n, *e;
  ...
  if (!p_dup || !q_dup || !dmp1_dup || !dmq1_dup || !iqmp_dup || !n_dup || !e_dup) {
    result = -1;
    goto __fail;
  }
  ...
  result = result || write_bn(0, p_dup, 128);
  result = result || write_bn(1, q_dup, 128);
  result = result || write_bn(2, dmp1_dup, 128);
  result = result || write_bn(3, dmq1_dup, 128);
  result = result || write_bn(4, iqmp_dup, 128);
  result = result || write_bn(5, n_dup, 256);
  result = result || write_bn(6, e_dup, 1);
