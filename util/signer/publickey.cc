...
size_t PublicKey::nwords() {
    const RSA* rsa = EVP_PKEY_get1_RSA(key_);
    const BIGNUM* n;
    RSA_get0_key(rsa, &n, nullptr, nullptr);

    size_t result = (BN_num_bytes(n) + 3) / 4;
    return result;
}
...
void PublicKey::print(const char* tag) {
  ...
  const BIGNUM* n;
  RSA_get0_key(rsa, &n, nullptr, nullptr);
  print(tag, rwords(), const_cast<BIGNUM*>(n));
}
...
void PublicKey::modToArray(uint32_t* dst, size_t nwords) {
  ...
  const BIGNUM* n;
  RSA_get0_key(rsa, &n, nullptr, nullptr);
  toArray(dst, nwords, const_cast<BIGNUM*>(n));
}

int PublicKey::encrypt(uint8_t* msg, int msglen, uint8_t* out) {
  const RSA* rsa = EVP_PKEY_get1_RSA(key_);
  int result = RSA_public_encrypt(msglen, msg, out, const_cast<RSA*>(rsa), RSA_PKCS1_OAEP_PADDING);
  return result;
}

int PublicKey::decrypt(uint8_t* msg, int msglen, uint8_t* out) {
  struct rsa_st* rsa = EVP_PKEY_get1_RSA(key_);
  int result =
...
int PublicKey::raw(uint8_t* in, int inlen, BIGNUM** out) {
  ...
  int result = BN_mod_exp(r, m, d, n, ctx);
  if (result == 1) {
    *out = BN_dup(r);
  }
...
int PublicKey::sign(const void* msg, size_t msglen, BIGNUM** output) {
  ...
  if (BN_mod(tmp, tmp, n, bnctx) != 1) goto __fail;
